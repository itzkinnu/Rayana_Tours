...
$apiEndpoint = '/api/Tour/touroption';
...
error_log('[touroption_proxy] payload=' . json_encode($payload));
...
error_log('[touroption_proxy] upstream_status=' . $httpCode . ' curlErr=' . ($curlErr ?: '')); 
if ($response === false) {
    error_log('[touroption_proxy] upstream_response=false');
} else {
    // Truncate long responses for log
    error_log('[touroption_proxy] upstream_response_snippet=' . substr($response, 0, 200));
}
